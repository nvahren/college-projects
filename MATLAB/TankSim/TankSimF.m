...
%Gameplay
turn = 1; %Which player starts. randi(2) is a random player.
...
plot(handles.plotAxeshandles.plotAxes, xTerrain, yTerrain, 'k-', 'LineWidth', 2)
hold on
...
% Place Objects
plot(handles.plotAxeshandles.plotAxes, xLocationP1, yLocationP1, 'ro')
hold on
plot(handles.plotAxeshandles.plotAxes, xLocationP2, yLocationP2, 'bo')
hold on
axis([0 100 0 5])
...
while gameComplete == 0
    ...
    if turn == 1
        ...
        clc
        clf
        plot(handles.plotAxesxTerrain, yTerrain, 'k-', 'LineWidth', 2)
        hold on
        area(xTerrain,yTerrain, 'FaceColor', 'g')
        hold on
        plot(handles.plotAxesxLocationP1, yLocationP1, 'ro')
        hold on
        plot(handles.plotAxesxLocationP2, yLocationP2, 'bo')
        hold on
        axis([0 100 0 5])
        ...
        while turnComplete == 0
            ...
            elseif strcmpi(action,'fire')
                ...
                while impact == 0
                    ...
                    plot(handles.plotAxesxProjectile(1:speedBoost:frame), yProjectile(1:speedBoost:frame), 'r-')
                    axis([0 100 0 5])
                    drawnow
                    hold on
                ...
                plot(handles.plotAxesxImpact, yImpact, 'rx')
                for rExp = 1:5
                    for exp = 1:360
                    ...
                    pause(0.1)
                    plot(handles.plotAxesxExp,yExp,'r-')
                end
                axis([0 100 0 5])
        ...
        while turnComplete == 0
            ...
            elseif strcmpi(action,'fire')
                ...
                while impact == 0
                    xProjectile(frame+1) = xProjectile(frame) + (powerFire*powerScale*cosd(angleFire)*frame)/framerate;
                    yProjectile(frame+1) = yProjectile(frame) + (powerFire*powerScale*frame - grav*frame^2)/framerate;
                    plot(handles.plotAxesxProjectile(1:speedBoost:frame), yProjectile(1:speedBoost:frame), 'b-')
                    axis([0 100 0 5])
                    drawnow
                    hold on
                ...
                plot(handles.plotAxesxImpact, yImpact, 'bx')
                for rExp = 1:5
                    for exp = 1:360
                        xExp(exp) = xImpact + (rExp*cosd(exp))/6;
                        yExp(exp) = yImpact + (rExp*sind(exp))/20;
                    ...
                    pause(0.1)
                    plot(handles.plotAxesxExp,yExp,'b-')
                end
                axis([0 100 0 5])
